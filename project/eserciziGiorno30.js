...
const subtitleText = `Al momento disponiamo dei Dati di ${countries.length} Nazioni`;
const feedbackText = `Le Nazioni che soddisfano il criterio di ricerca sono `;
...
const switchStatusGraph = initSwitchStatusGraph();
...
function initSwitchStatusGraph() {
    let switchStatusGraph = {};
    let propertiesStrings = [];
    const HTMLElementes = document.querySelectorAll(".graphButton");
...
function searchCountries(countriesArray=countries, reverse=false, inputString) {
    let resArr = [];
    const searchCriteria = getSearchCriteria();

    if(searchCriteria === "languages") {
        resArr = countriesArray.filter(elem => elem.languages.some(string => string.toLowerCase().startsWith(inputString.toLowerCase())));
    }
    else {
        resArr = countriesArray.filter(elem => elem[searchCriteria] ? elem[searchCriteria].toLowerCase().startsWith(inputString.toLowerCase()) : null);
    }
...
function getSearchCriteria() {
    ...
    switch(criteria) {
        case "Ricerca per Nazione":
            return "name";
        case "Ricerca per Capitale":
            return "capital";
        case "Ricerca per Lingua":
            return "languages";
        default:
            return "name";
    }
...
function applySortCriteria(countriesArray, sortCriteria, reverse) {
    countriesArray.sort((a, b) => (a[sortCriteria] ? a[sortCriteria] : "Zagreb1") > b[sortCriteria] ? 1 : -1);

    if(reverse)     countriesArray.reverse();

    return countriesArray;
}

function fillCountriesDiv(countriesArray=countries) {
    countriesDiv.innerHTML = "";

    for(const country of countriesArray) {
        const countryDiv = createCountryBox(country);
...
function createCountryBox(country) {
    ...
    const flagP = createFlagP(country.flag);
    const countryNameP = createCountryNameP(country.name);
    const countryInfoDiv = createCountryInfoDiv(country);

    countryDiv.appendChild(flagP);
...
function createFlagP(flag) {
    const flagP = document.createElement("img");
    flagP.classList.add("flagP");
...
function createCountryNameP(countryName) {
    const countryNameP = document.createElement("p");
    countryNameP.classList.add("countryNameP");

    countryNameP.textContent = countryName.toUpperCase();
...
function createCountryInfoDiv(country) {
    ...
    countryInfoDiv.classList.add("countryInfoDiv");


    const capitalP = document.createElement("p");
    capitalP.classList.add("countryInfoElement");
    capitalP.textContent = `Capitale: ${country.capital ? country.capital : "N.D."}`;

    const languagesP = document.createElement("p");
    languagesP.classList.add("countryInfoElement");
    languagesP.textContent = `${country.languages.length > 1 ? "Lingue" : "Lingua"}: ${country.languages.join(", ")}`;

    const populationP = document.createElement("p");
    populationP.classList.add("countryInfoElement");
    populationP.textContent = `Popolazione: ${country.population}`;

    const areaP = document.createElement("p");
    areaP.classList.add("countryInfoElement");
    areaP.textContent = `Area (km2): ${country.area ? country.area : "N.D."}`;

    const regionP = document.createElement("p");
    regionP.classList.add("countryInfoElement");
    regionP.textContent = `Regione Geografica: ${country.region}`;


    countryInfoDiv.appendChild(capitalP);
    countryInfoDiv.appendChild(languagesP);
    countryInfoDiv.appendChild(populationP);
    countryInfoDiv.appendChild(areaP);
    countryInfoDiv.appendChild(regionP);
...
function funzioneMaster() {
    ...
    if(!inputString) {
        feedback.innerHTML = "";
        fillCountriesDiv(applySortCriteria(countries, getSortCriteria(), reverseSortStatus));
        displayGraph(countries);
        return;
    }

    const resCountriesArr = searchCountries(countries, reverseSortStatus, inputString);

    displayGraph(resCountriesArr);
    fillCountriesDiv(resCountriesArr);
}

function displayGraph(countriesArray, switchGraph=switchStatusGraph) {
    let graphOption;

    for(const key in switchGraph) {
        if(switchGraph[key]) {
            graphOption = key;
        }
    }
...
function mostSpokenLanguages(countriesArray, count) {
    ...
    for(let country of countriesArray) {
        for(let language of country.languages) {
            tempIndex = res.findIndex(element => element.language === language.toUpperCase());
            res[tempIndex] ? res[tempIndex].count++ : res.push({ language: language.toUpperCase(), count: 1 });
        }
    }
...
function mostPopulatedCountries(countriesArray, count) {
    return countriesArray.sort((a, b) => (a.population > b.population) ? -1 : 1).slice(0, count);
}

function displayMostPopulatedCountries(countriesArray) {
    graph.innerHTML = "";
    graphTitle.textContent = graphTitlePopulation;

    const populatedCountries = mostPopulatedCountries(countriesArray, countriesArray.length);

    for(const country of populatedCountries) {
        const graphEntryDiv = createGraphEntryDiv(country.name, country.population, populatedCountries[0].population);
        graph.appendChild(graphEntryDiv);
    }
}

function displayMostSpokenLanguages(countriesArray) {
    graph.innerHTML = "";
    graphTitle.textContent = graphTitleLanguages;

    const spokenLanguages =  mostSpokenLanguages(countriesArray, countriesArray.length);

    for(const language of spokenLanguages) {
        const graphEntryDiv = createGraphEntryDiv(language.language, language.count, spokenLanguages[0].count);
        graph.appendChild(graphEntryDiv);
    }
}

function createGraphEntryDiv(entryName, currentValue, maxValue) {
    const graphEntryDiv = document.createElement("div");
    graphEntryDiv.classList.add("graphEntryDiv");


    const graphEntryNameP = document.createElement("p");
    graphEntryNameP.classList.add("graphEntryNameP");
    graphEntryNameP.textContent = entryName;

    const graphBarDiv = createBar(maxValue, currentValue);

    const graphStatP = document.createElement("p");
    graphStatP.classList.add("graphStatP");
    graphStatP.textContent = currentValue;


    graphEntryDiv.appendChild(graphEntryNameP);
    graphEntryDiv.appendChild(graphBarDiv);
    graphEntryDiv.appendChild(graphStatP);

    return graphEntryDiv;
}

function createBar(max, current) {
    const percentage = (+current / +max).toFixed(4);

    const graphBarDiv = document.createElement("div");
    graphBarDiv.classList.add("graphBarDiv");

    const graphBarP = document.createElement("p"); 
    graphBarP.classList.add("graphBarP");
    graphBarP.innerHTML = ``;
    graphBarP.style.width = `${percentage * 100}%`;

    graphBarDiv.appendChild(graphBarP);
